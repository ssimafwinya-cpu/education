...
export function globalSearch(state: AppState, query: string, limit = 20): SearchHit[] {
  ...
  for (const s of state.subjects) {
    if (match(s.name) || match(s.goal ?? "")) {
      hits.push({ type: "subject", id: s.id, title: `${s.emoji} ${s.name}`, snippet: s.goal ?? "Subject", href: `/app/courses/${s.id}` });
    }
  }
  for (const n of state.notes) {
    if (match(n.title) || match(n.content)) {
      ...
      hits.push({ type: "note", id: n.id, title: n.title, snippet, href: `/app/notes?id=${n.id}` });
    }
  }
  for (const d of state.decks) {
    if (match(d.name) || match(d.description)) {
      hits.push({ type: "deck", id: d.id, title: `${d.emoji} ${d.name}`, snippet: d.description || "Deck", href: `/app/flashcards/${d.id}` });
    }
  }
  for (const c of state.cards) {
    if (match(c.front) || match(c.back)) {
      hits.push({ type: "card", id: c.id, title: c.front.slice(0, 50), snippet: c.back.slice(0, 60), href: `/app/flashcards/${c.deckId}` });
    }
  }
  for (const qz of state.quizzes) {
    if (match(qz.title) || match(qz.description)) {
      hits.push({ type: "quiz", id: qz.id, title: qz.title, snippet: qz.description || "Quiz", href: `/app/quizzes/${qz.id}` });
    }
  }
