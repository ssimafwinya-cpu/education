  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <SettingsSection icon={<Sparkles size={17} />} title="AI providers">
        <p className="text-sm text-ink-muted">Cognify works out of the box with a built-in offline tutor. Add an API key on the server (see <code className="rounded bg-surface px-1 py-0.5 text-xs">.env</code>) to unlock live LLM tutoring.</p>
      ...
      <SettingsSection icon={<Database size={17} />} title="Data & privacy">
        ...
        <div className="mt-4 rounded-xl border border-rose-500/20 bg-rose-500/5 p-3">
          ...
          <p className="mt-1 text-xs text-ink-muted">Permanently erase all your Cognify data from this device.</p>
          <ConfirmButton
            onConfirm={() => { localStorage.removeItem("cognify.state.v1"); dispatch({ type: "RESET" }); toast({ emoji: "🗑️", title: "All data erased" }); }}
  ...
  const onFile = async (file: File) => {
    try {
      ...
      if (parsed?.version === 1 && parsed.profile && Array.isArray(parsed.cards)) {
        ...
        onClose();
      } else setError("This file doesn't look like a Cognify backup.");
    } catch {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Import data">
      <div className="space-y-3">
        <p className="text-sm text-ink-muted">Select a Cognify backup file (.json). This will replace your current data.</p>
