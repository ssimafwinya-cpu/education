...
export default function CourseDetail() {
  ...
  if (!subject || !st) {
    return (
      <EmptyState icon="🔍" title="Course not found" description="This subject may have been deleted." action={<Link href="/app/courses" className="btn-primary">Back to courses</Link>} />
    );
  }
  ...
  const addNote = () => {
    ...
    router.push(`/app/notes?id=${note.id}`);
  };

  return (
    <div>
      <Link href="/app/courses" className="btn-ghost btn-sm mb-4 -ml-2"><ArrowLeft size={15} /> Courses</Link>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <Card className="relative overflow-hidden">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {st.dueCards > 0 && <Link href="/app/review" className="btn-primary btn-sm"><Brain size={14} /> Review {st.dueCards} due</Link>}
            <Link href={`/app/tutor?subject=${id}`} className="btn-secondary btn-sm"><MessageSquare size={14} /> Ask AI about {subject.name}</Link>
          </div>
        </Card>
      </motion.div>
        ...
        {decks.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {decks.map((d) => {
              ...
              return (
                <Link key={d.id} href={`/app/flashcards/${d.id}`}>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
          ...
          {notes.length === 0 ? (
            ...
            <div className="space-y-2">
              {notes.slice(0, 5).map((n) => (
                <Link key={n.id} href={`/app/notes?id=${n.id}`}>
        ...
        <section>
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link href="/app/quizzes" className="btn-ghost btn-sm"><Plus size={14} /> New quiz</Link>
          </div>
          {quizzes.length === 0 ? (
