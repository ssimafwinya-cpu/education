          ...
          <div className="space-y-3">
            <Card>
              ...
              <div className="space-y-2.5">
                ...
                <QuizAction text={doc.text} docName={doc.name} onDone={(id) => router.push(`/hub/quizzes/${id}`)} />
                <SummaryAction text={doc.text} docName={doc.name} onDone={(id) => router.push(`/hub/notes?id=${id}`)} />
                <button
                  onClick={async () => {
...
function HighlightableText({ doc }: { doc: Extracted }) {
  ...
  const explain = () => {
    if (!selection) return;
    router.push(`/hub/tutor?q=${encodeURIComponent(`Explain this from ${doc.name}: "${selection.text.slice(0, 300)}"`)}`);
  };
