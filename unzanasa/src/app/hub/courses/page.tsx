              ...
              <motion.div key={s.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
                <Card hover className="group relative h-full">
                  <div className="flex items-start justify-between">
                    <Link href={`/hub/courses/${s.id}`} className={cn("grid h-12 w-12 place-items-center rounded-xl text-2xl", c.bg)}>{s.emoji}</Link>
                  ...
                  <Link href={`/hub/courses/${s.id}`} className="mt-3 block">
