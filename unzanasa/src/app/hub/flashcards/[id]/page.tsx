...
export default function DeckDetail() {
  ...
  if (!deck) {
    return <EmptyState icon="🔍" title="Deck not found" action={<Link href="/hub/flashcards" className="btn-primary">Back to decks</Link>} />;
  }
  ...
  return (
    <div>
      <Link href="/hub/flashcards" className="btn-ghost btn-sm mb-4 -ml-2"><ArrowLeft size={15} /> Decks</Link>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              {subj && <Link href={`/hub/courses/${subj.id}`}><Badge tone="teal">{subj.emoji} {subj.name}</Badge></Link>}
            </div>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          {due > 0 && <Link href={`/hub/review?deck=${id}`} className="btn-primary"><Brain size={16} /> Review ({due})</Link>}
