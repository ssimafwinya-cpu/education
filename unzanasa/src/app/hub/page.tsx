  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Link href="/hub/review" className="btn-primary">
          ...
          <motion.div {...fade} transition={{ delay: 0.1 }}>
            <Card>
              <div className="mb-4 flex items-center justify-between">
                ...
                <Link href="/hub/planner" className="btn-ghost btn-sm">Open planner <ChevronRight size={14} /></Link>
              </div>
              {todayTasks.length === 0 ? (
                <div className="rounded-xl border border-dashed border-edge-strong p-5 text-center text-sm text-ink-muted">
                  No tasks scheduled today.{" "}
                  <Link href="/hub/planner" className="font-semibold text-brand-500 hover:underline">Auto-generate a plan →</Link>
                </div>
              ) : (
                <>
          ...
          <motion.div {...fade} transition={{ delay: 0.15 }}>
            <Card>
              <div className="mb-3 flex items-center justify-between">
                ...
                <Link href="/hub/analytics" className="btn-ghost btn-sm">Analytics <ChevronRight size={14} /></Link>
              </div>
              <AreaChart
              ...
              <Card>
                ...
                <div className="space-y-3">
                  {weakest.map((m) => {
                    ...
                    return (
                      <Link key={m.subject.id} href={`/hub/courses/${m.subject.id}`} className="flex items-center gap-3 rounded-xl border border-edge p-3 transition hover:border-edge-strong">
          ...
          <motion.div {...fade} transition={{ delay: 0.3 }}>
            <Card>
              <div className="mb-3 flex items-center justify-between">
                ...
                <Link href="/hub/notes" className="text-xs text-brand-500 hover:underline">All</Link>
              </div>
              <div className="space-y-1">
                {recentNotes.map((n) => (
                  <Link key={n.id} href={`/hub/notes?id=${n.id}`} className="block rounded-lg px-2 py-1.5 transition hover:bg-surface">
      ...
      {pinned.length > 0 && (
        <motion.div {...fade} transition={{ delay: 0.25 }} className="mt-5">
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link href="/hub/courses" className="btn-ghost btn-sm">All courses <ChevronRight size={14} /></Link>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {pinned.map((s) => {
              ...
              return (
                <Link key={s.id} href={`/hub/courses/${s.id}`}>
...
function AskBar() {
  ...
  return (
    <motion.form
      {...fade}
      onSubmit={(e) => { e.preventDefault(); if (q.trim()) router.push(`/hub/tutor?q=${encodeURIComponent(q.trim())}`); }}
      className="mb-5"
    >
...
function QuickActions() {
  const actionsList = [
    { icon: Layers, label: "Generate AI Flashcards", href: "/hub/flashcards", tone: "brand" },
    { icon: CalendarDays, label: "Create your Study Plan", href: "/hub/planner", tone: "teal" },
    { icon: FileText, label: "Upload a file", href: "/hub/pdf", tone: "amber" },
    { icon: Lightbulb, label: "Understand a topic", href: "/hub/tutor", tone: "violet" },
  ];
  return (
  ...
  return (
    <motion.div {...fade} className="mb-5">
      <div className="mb-3 flex items-center justify-between">
        ...
        <Link href="/hub/review" className="btn-ghost btn-sm">Review all <ChevronRight size={14} /></Link>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {queue.map(({ deck, subject, dueN, xp }) => (
          <Link key={deck.id} href={`/hub/review?deck=${deck.id}`}>
