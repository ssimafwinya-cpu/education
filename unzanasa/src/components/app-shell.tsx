...
const NAV_SECTIONS: { title: string; items: NavItem[] }[] = [
  {
    title: "",
    items: [{ href: "/hub", label: "Dashboard", icon: LayoutDashboard, exact: true }],
  },
  {
    title: "Science Labs",
    items: [{ href: "/hub/science", label: "Science Labs", icon: FlaskConical }],
  },
  {
    title: "Learn",
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/hub/exams", label: "Exams", icon: GraduationCap },
    ],
  },
  {
    title: "Plan & AI",
    items: [
      { href: "/hub/tutor", label: "AI Tutor", icon: MessageSquare },
      { href: "/hub/planner", label: "Planner", icon: CalendarDays },
    ],
  },
  {
    title: "Progress",
    items: [
      { href: "/hub/analytics", label: "Analytics", icon: LineChart },
      { href: "/hub/achievements", label: "Achievements", icon: Trophy },
      { href: "/hub/social", label: "Social", icon: Users },
    ],
  },
];
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const sidebar = (
    <div className="flex h-full flex-col">
      <Link href="/hub" className="flex items-center gap-2.5 px-2 py-1">
        <div className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-brand-500 to-forest-light text-white shadow-glow">
          <GraduationCap size={20} />
        </div>
        <div>
          <div className="text-base font-bold leading-none tracking-tight">Academic Hub</div>
          <div className="text-[11px] text-ink-faint">UNZANASA · Natural Sciences</div>
        </div>
      </Link>
      <Link href="/" className="mt-2 flex items-center gap-2 rounded-lg px-2 py-1.5 text-xs text-ink-faint transition hover:bg-surface hover:text-ink">
        <ArrowLeft size={13} /> Back to UNZANASA site
      </Link>
      ...
      <nav className="mt-5 flex-1 space-y-3 overflow-y-auto pr-1">
        ...
        {state.profile.role === "admin" && (
          <div>
            ...
            <Link href="/hub/admin" className={cn("nav-link", isActive("/hub/admin") && "nav-link-active")}>
      ...
      <Link
        href="/hub/settings"
        className={cn("nav-link mt-2", isActive("/hub/settings") && "nav-link-active")}
      >
        <Settings size={18} />
        <span>Settings</span>
      </Link>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-edge bg-surface/70 px-4 py-3 backdrop-blur-xl">
          ...
          <div className="ml-auto flex items-center gap-2">
            <SyncBadge />
            <Link href="/hub/tutor" className="btn-primary btn-sm hidden sm:inline-flex">
              <Sparkles size={15} /> Ask AI
            </Link>
            <button onClick={cycleTheme} className="btn-ghost btn-sm" aria-label="Toggle theme" title={`Theme: ${theme}`}>
              <ThemeIcon size={18} />
            </button>
            <Link href="/hub/settings" className="grid h-9 w-9 place-items-center rounded-full bg-gradient-to-br from-brand-500/20 to-teal-500/20 text-lg" title={state.profile.name}>
